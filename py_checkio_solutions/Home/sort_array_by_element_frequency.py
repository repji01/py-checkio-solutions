#!/home/mattie/.local/bin/checkio --domain=py run sort-array-by-element-frequency
...
def frequency_sort(items: iter) -> list:
    """Sort an array of elements by frequency, in decreasing order.

    Args:
        items: An array of items.
    Returns:
        grouped: A sorted list of items.
    """
    count = sorted(Counter(items).items(), key=lambda x: -x[1])
    ans = [[k] * v for (k , v) in count]
